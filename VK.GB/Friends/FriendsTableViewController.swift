...
class FriendsTableViewController: UITableViewController {
    ...
    var friends: Results<Friends> = {
        let realm = try! Realm()
        return realm.objects(Friends.self).sorted(byKeyPath: "lastName", ascending: true)
    }()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.imageFriend.layer.cornerRadius = 25
        cell.imageFriend.clipsToBounds = true
        
        let getCacheImage = GetCacheImage(url: friend.imageUrl)
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let share = UITableViewRowAction(style: .default, title: "Поделиться") { (action, indexPath) in
            ...
            if let image = self.friends[indexPath.row].imageUrl {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "photoSegue" {
            if let indexPath = tableView.indexPathForSelectedRow {
                ...
                dvc.idFriend = friends[indexPath.row].idFriend
                dvc.userId = userId
            }
        }
    }
